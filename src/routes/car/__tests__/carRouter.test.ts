...
describe('Car Route', () => {
  ...
  describe('Car findAll', () => {
      ...
      it("Should return error when service throws", async () => {
        (carService.findAll as Mock).mockResolvedValue(new ServiceResponse(ResponseStatus.Failed, "Error finding all cars",null,StatusCodes.INTERNAL_SERVER_ERROR))
        const response = await request(app).get('/cars');
        const responseBody: ServiceResponse<Car[]> = response.body;

        expect(responseBody.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
  ...
  describe('Car findById', () => {
    ...
    it("Should return error when service throws", async () => {
      const testId = 1;
      (carService.findById as Mock).mockResolvedValue(new ServiceResponse(ResponseStatus.Failed,`Error finding user with id ${testId}`,null,StatusCodes.INTERNAL_SERVER_ERROR))
      const response = await request(app).get(`/cars/${testId}`);
      const responseBody: ServiceResponse<Car> = response.body;

      expect(responseBody.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
      expect(responseBody.success).toBeFalsy();
      expect(responseBody.message).toContain(`Error finding user with id ${testId}`);
      expect(responseBody.responseObject).toBeNull();
    })
  });
  ...
  describe('POST /cars', () => {
    it('should create a car and return a successful response', async () => {
      const mockCar = { brand: 'Test Brand', color: 'Test Color', plate: 'ABC-1234' };
      const mockCreateCarDTO = { brand: mockCar.brand, color: mockCar.color, plate: mockCar.plate };
      ...
      const response = await request(app)
        .post('/cars')
        .send(mockCreateCarDTO) // Send request body
    ...
    it('should handle errors during creation and return an error response', async () => {
      const mockCreateCarDTO = { brand: 'Test Brand', color: 'Test Color', plate: 'ABC-1234' };
      (carService.create as Mock).mockResolvedValue(
        new ServiceResponse(
...
function mockFindAll(){
  (carService.findAll as Mock).mockReturnValue(new ServiceResponse(
    ResponseStatus.Success,"Cars found", mockCars, StatusCodes.OK
  ));
}
function mockFindOne(){
  (carService.findById as Mock).mockReturnValue(new ServiceResponse(
    ResponseStatus.Success,"Car found", mockCars[0], StatusCodes.OK
  ));
}
